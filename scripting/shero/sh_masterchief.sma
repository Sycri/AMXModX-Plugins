...
#if PLAYER_MODEL > 0
	new bool:gModelPlayerSet[SH_MAXSLOTS+1]
...
public plugin_init() {
	...
	new pcvarGravity = register_cvar("masterchief_gravity", "1.0")
	new pcvarSpeed = register_cvar("masterchief_speed", "-1")
	new pcvarP90Mult = register_cvar("masterchief_p90mult", "1.3")
	
	#if PLAYER_MODEL == 1
		gPcvarTeamGlow = register_cvar("masterchief_teamglow", "0")
	#endif
...
public plugin_precache() {
		...
		gModelPlayerLoaded = true
			...
			if(file_exists(gModelPlayer)) {
				...
				log_amx("[SH](%s)Aborted loading ^"%s^", file does not exist on server", gHeroName, gModelPlayer)
				...
				if(file_exists(gModelPlayer_T)) {
					...
					log_amx("[SH](%s)Aborted loading ^"%s^", file does not exist on server", gHeroName, gModelPlayer_T)
					gModelPlayerLoaded = false
				}
				...
				if(file_exists(gModelPlayer_CT)) {
					...
					log_amx("[SH](%s)Aborted loading ^"%s^", file does not exist on server", gHeroName, gModelPlayer_CT)
					gModelPlayerLoaded = false
				}
			#endif
		#endif
		...
		if(file_exists(gModel_V_P90)) {
			...
			log_amx("[SH](%s)Aborted loading ^"%s^", file does not exist on server", gHeroName, gModel_V_P90)
		...
		if(file_exists(gModel_P_P90)) {
			...
			log_amx("[SH](%s)Aborted loading ^"%s^", file does not exist on server", gHeroName, gModel_P_P90)
			gModelWeaponLoaded = false
		}
	#endif
}
	...
	if(is_user_alive(id)) {
		switch(mode) {
			case SH_HERO_ADD: {
				...
				#if defined USE_WPN_MODEL
					if(gModelWeaponLoaded && get_user_weapon(id) == CSW_P90) switch_model(id)
				#endif
				
				#if PLAYER_MODEL > 0
					if(gModelPlayerLoaded) masterchief_tasks(id)
				#endif
			}
			case SH_HERO_DROP: {
				if(gHasMasterChief[id]) {
					...
					#if PLAYER_MODEL > 0
						if(gModelPlayerLoaded) masterchief_unmorph(id)
					#endif
				}
			}
		}
...
public weapon_change(id) {
	if(!sh_is_active() || !gHasMasterChief[id]) return
	
	if(read_data(2) != CSW_P90) return
	
	#if defined USE_WPN_MODEL
		if(gModelWeaponLoaded) switch_model(id)
	#endif
...
switch_model(id) {
	...
	set_pev(id, pev_viewmodel2, gModel_V_P90)
	set_pev(id, pev_weaponmodel2, gModel_P_P90)
}
#endif
//----------------------------------------------------------------------------------------------
#if PLAYER_MODEL > 0
masterchief_tasks(id) {
	set_task(1.1, "masterchief_morph", id)
	
	#if PLAYER_MODEL == 1
		if(get_pcvar_num(gPcvarTeamGlow)) set_task(1.0, "masterchief_glow", id+100, _, _, "b")
	#endif
}
//----------------------------------------------------------------------------------------------
public masterchief_morph(id) {
	if(gModelPlayerSet[id] || !is_user_alive(id) || !gHasMasterChief[id]) return
	...
	show_hudmessage(id, "%s - Spartan-117 reporting for duty", gHeroName)

	gModelPlayerSet[id] = true
}
//----------------------------------------------------------------------------------------------
masterchief_unmorph(id) {
	if(gModelPlayerSet[id] && is_user_connected(id)) {
		if(is_user_alive(id)) {
			set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, -1)
			show_hudmessage(id, "%s - MODE OFF, you returned to normal self", gHeroName)
		}
...
//----------------------------------------------------------------------------------------------
masterchief_sound(id) {
	emit_sound(id, CHAN_AUTO, gMasterChiefSound, 0.2, ATTN_NORM, SND_STOP, PITCH_NORM)
	emit_sound(id, CHAN_AUTO, gMasterChiefSound, 0.2, ATTN_NORM, 0, PITCH_NORM)
}
//----------------------------------------------------------------------------------------------
#if PLAYER_MODEL == 1
...
public sh_client_death(id) {
	if(!sh_is_active() || is_user_alive(id) || !gHasMasterChief[id]) return
