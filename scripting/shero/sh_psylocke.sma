...
public plugin_init() {
	...
	//LOOP
	set_task(0.1, "psylocke_loop", _, _, _, "b")
...
public psylocke_loop() {
	...
	static players[SH_MAXSLOTS], playerCount, player, i, e, enemy, closestEnemy
	...
	for(i = 0; i < playerCount; i++) {
		player = players[i]
		if(is_user_bot(player) || !gHasPsylocke[player]) continue
		...
		for(e = 0; e < playerCount; e++) {
			enemy = players[e]
			if(playerTeam == cs_get_user_team(enemy)) continue
		...
		switch(floatround(closestEnemy * 0.0254)) {
			case 0..10: {
				...
				ShowSyncHudMsg(player, gMsgSync, "DANGER!  Enemy Detected Within 10 Meters!!!")
			}
			case 11..20: {
				set_hudmessage(255, 155, 0, 0.01, 0.27, 1, 6.0, 0.5, 0.1, 0.1, 4)
				ShowSyncHudMsg(player, gMsgSync, "WARNING! Enemy Detected Within 20 Meters!!")
			}
			case 21..30: {
				set_hudmessage(255, 255, 255, 0.01, 0.27, 1, 6.0, 0.5, 0.1, 0.1, 4)
				ShowSyncHudMsg(player, gMsgSync, "CAUTION!  Enemy Detected Within 30 Meters!")
			}
		}
	}
}
