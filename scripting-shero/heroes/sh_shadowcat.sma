...
new bool:gHasShadowcat[MAX_PLAYERS + 1];
new gShadowcatTimer[MAX_PLAYERS + 1];
new gmsgSync;
...
public plugin_init()
{
	...
	new pcvarLevel = create_cvar("shadowcat_level", "0", .has_min = true, .min_val = 0.0);
	bind_pcvar_float(create_cvar("shadowcat_cooldown", "30", .has_min = true, .min_val = 0.0), CvarCooldown);
	bind_pcvar_num(create_cvar("shadowcat_cliptime", "6", .has_min = true, .min_val = 0.0), CvarClipTime);
	...
	// LOOP
	set_task_ex(1.0, "@Task_ShadowcatLoop", _, _, _, SetTask_Repeat);
...
public sh_hero_init(id, heroID, mode)
{
	...
	switch (mode) {
		case SH_HERO_ADD: {
			...
			// Make sure loop doesn't fire for them
			gShadowcatTimer[id] = -1;
		}
		case SH_HERO_DROP: {
			gHasShadowcat[id] = false;

			if (gShadowcatTimer[id] >= 0)
...
public sh_hero_key(id, heroID, key)
{
	...
	if (key == SH_KEYDOWN) {
		...
		gShadowcatTimer[id] = CvarClipTime;
	...
	for (i = 0; i < playerCount; ++i) {
		...
		if (gHasShadowcat[player]) {
			...
			if (noclipTime > 0) {
				...
				if (cooldown > 0.0)
					sh_set_cooldown(player, cooldown);
